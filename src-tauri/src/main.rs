...
use wtrlab_offline_lib::{ext_mod, tauri_generate_context};
...
fn main() -> Result<Infallible, Box<dyn Error>> {
    ...
    // 👉 Equivalent to `python -m wtrlab_offline`,
    // i.e, run the `src-tauri/python/wtrlab_offline/__main__.py`
    let py_script = PythonScript::Module("wtrlab_offline".into());
